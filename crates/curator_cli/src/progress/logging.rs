...
impl LoggingReporter {
    ...
    fn log_fetch_event(event: SyncProgress) {
        match event {
    ...
    fn log_star_event(&self, event: SyncProgress) {
        match event {
            ...
            SyncProgress::StarredRepo {
                ...
                let repo = Self::repo_label(&owner, &name);
                if already_starred {
                    tracing::debug!(repo = %repo, "Already starred");
                } else {
                    tracing::info!(repo = %repo, "Starred");
                }
            }
            SyncProgress::StarError { owner, name, error } => {
                tracing::warn!(
                    repo = %Self::repo_label(&owner, &name),
                    error = %error,
                    "Failed to star"
                );
            }
    ...
    fn log_persistence_event(&self, event: SyncProgress) {
        match event {
            ...
            SyncProgress::Persisted { owner, name } => {
                tracing::debug!(repo = %Self::repo_label(&owner, &name), "Saved to database");
            }
    ...
    fn log_prune_event(&self, event: SyncProgress) {
        match event {
            ...
            SyncProgress::PrunedRepo { owner, name } => {
                tracing::debug!(repo = %Self::repo_label(&owner, &name), "Pruned");
            }
    ...
    pub fn handle(&self, event: SyncProgress) {
        match event {
            ...
            | event @ SyncProgress::PageFetchRetry { .. } => Self::log_fetch_event(event),
            ...
            | event @ SyncProgress::RateLimitBackoff { .. } => self.log_star_event(event),
            ...
            | event @ SyncProgress::SyncNamespacesComplete { .. } => {
                self.log_persistence_event(event);
            }
            event @ SyncProgress::PruningRepos { .. }
            | event @ SyncProgress::PrunedRepo { .. }
            | event @ SyncProgress::PruneError { .. }
            | event @ SyncProgress::PruningComplete { .. } => self.log_prune_event(event),
